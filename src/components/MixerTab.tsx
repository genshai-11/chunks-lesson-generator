...
export default function MixerTab() {
  ...
  const handleAIGenerate = async () => {
    ...
    const batchSize = 5;
    const totalBatches = Math.ceil(aiQuantity / batchSize);

    try {
      for (let i = 0; i < totalBatches; i++) {
        const currentQuantity = Math.min(batchSize, aiQuantity - i * batchSize);
        ...
        const validatedResults = results.map(res => {
          ...
          return {
            ...
            audioUrl: null // Defer audio generation to save API limits and speed up UI
          };
        });

        setAiResults(prev => [...prev, ...validatedResults]);
        setGenerationProgress(prev => ({ ...prev, current: prev.current + currentQuantity }));
      }
    } catch (error) {
      console.error(error);
      alert("Failed to auto-generate some chunks. See console for details.");
    } finally {
      setLoading(false);
