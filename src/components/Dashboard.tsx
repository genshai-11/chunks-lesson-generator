...
import CVRMeasureTab from "./CVRMeasureTab";
...
export default function Dashboard() {
  ...
  const tabs = [
    ...
    { id: "measure", label: "CVR Measure", icon: Mic },
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans">
      ...
      <div className="md:hidden h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 sticky top-0 z-20">
        <div className="flex items-center">
          ...
          <span className="ml-2 text-[10px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
            generator
          </span>
        </div>
        <button
      ...
      >
        <div
          className={`h-16 flex items-center border-b border-gray-100 ${isCollapsed ? "justify-center px-0" : "justify-between px-6"}`}
        >
          {!isCollapsed && (
            <div className="flex items-center overflow-hidden">
              ...
              <span className="ml-2 text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
                gen
              </span>
            </div>
          )}
        ...
        <nav
          className={`flex-1 overflow-y-auto py-6 space-y-2 ${isCollapsed ? "px-3" : "px-4"}`}
        >
          {!isCollapsed && (
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 ${isActive ? "text-red-600" : "text-gray-400"} ${isCollapsed ? "" : "mr-3"}`}
                />
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50">
          <div
            className={`flex items-center ${isCollapsed ? "flex-col space-y-4" : "justify-between"}`}
          >
            <div className="flex items-center overflow-hidden">
              <div
                className="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center text-red-600 flex-shrink-0"
      ...
      {/* MAIN CONTENT */}
      <main
        className={`flex-1 transition-all duration-300 ease-in-out pb-20 md:pb-0 min-h-screen ${isCollapsed ? "md:ml-20" : "md:ml-64"}`}
      >
        <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 animate-in fade-in duration-300">
          ...
          {activeTab === "chunks" && <ChunksTab aiSettings={aiSettings} />}
          {activeTab === "player" && <PlayerTab />}
          {activeTab === "measure" && <CVRMeasureTab resources={resources} />}
